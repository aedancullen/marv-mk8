...
public class AutopilotTrackerMsoSimple extends AutopilotTracker {
	...
	public AutopilotTrackerMsoSimple(DigitalChannel MbRx, AnalogInput MbX, AnalogInput MbY, double MbXOffset, double MbYOffset) {

		this.MbRx = MbRx;
		MbRx.setMode(DigitalChannel.Mode.OUTPUT);
		MbRx.setState(true); // CHANGED
		this.MbX = MbX;
		this.MbY = MbY;
	...
	public void update() {
		...
		//	MbRx.setState(true); CHANGED
		//	MbRx.setState(false); CHANGED
		//}

		robotPosition[0] = voltageToInches(MbX.getVoltage()) + MbXOffset;
		robotPosition[1] = voltageToInches(MbY.getVoltage()) + MbYOffset;

		/*long time = System.currentTimeMillis(); CHANGED down
